...
Class SparqlEngine extends Object{
    ...
    */
    public static function writeQueryResultAsHtmlTable($queryResult) {
        ...
        $html.= '<table border="1" cellpadding="3" cellspacing="0"><tr><td><b>No.</b></td>';
        foreach ($queryResult[0] as $varName => $value)
        $html.= "<td align='center'><b>$varName</b></td>";
        $html.= '</tr>';

        foreach ($queryResult as $n => $var) {


            $html.= '<tr><td width="20" align="right">' .($n + 1) .'.</td>';
            foreach ($var as $varName => $value) {
                if($value !=''){
                    $html.= INDENTATION . INDENTATION . '<td bgcolor="';
                    $html.= RDFUtil::chooseColor($value);
                    $html.= '">';
                    $html.= '<p>';
                   ...
                   // if (is_a($value, 'Literal')) {
                    ...
                    $html.=  RDFUtil::getNodeTypeName($value) .$value->getLabel() . $lang . $dtype .'</p>';
                }else{
                    $html.=  "<td bgcolor='white'>unbound";
                }
            }
            $html.=  '</tr>';
        }
